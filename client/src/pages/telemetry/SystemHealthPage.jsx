  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1 font-semibold">
              <Radio className="w-3 h-3 text-emerald-400 animate-pulse" /> Live Telemetry Pulse
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-5 border-slate-800 bg-slate-900/70 space-y-3">
          <div className="flex items-center justify-between text-slate-400 text-xs">
            <span>CPU Compute Load</span>
            <Cpu className="w-4 h-4 text-brand-400" />
          </div>
          <div className="text-2xl font-black text-white">{metrics.cpuUsage}%</div>
          ...
          <div className="text-[11px] text-slate-400">Multi-core Node runtime pool</div>
        </Card>
        ...
        <Card className="p-5 border-slate-800 bg-slate-900/70 space-y-3">
          ...
          <div className="text-2xl font-black text-purple-400">
            {metrics.memoryUsageMb} MB <span className="text-xs text-slate-500 font-normal">/ 2GB</span>
          </div>
          <div className="w-full h-1.5 rounded-full bg-slate-800 overflow-hidden">
            <div
          ...
          <div className="text-[11px] text-slate-400">Heap allocated & active cache buffers</div>
        </Card>
        ...
        <Card className="p-5 border-slate-800 bg-slate-900/70 space-y-3">
          ...
          <div className="text-2xl font-black text-emerald-400">{metrics.avgLatencyMs} ms</div>
          <div className="w-full h-1.5 rounded-full bg-slate-800 overflow-hidden">
            <div
          ...
          <div className="text-[11px] text-emerald-400/80 font-medium">Sub-50ms SLA Guaranteed</div>
        </Card>
        ...
        <Card className="p-5 border-slate-800 bg-slate-900/70 space-y-3">
          ...
          <div className="text-2xl font-black text-amber-400">
            {metrics.dbPoolActive} <span className="text-xs text-slate-500 font-normal">/ {metrics.dbPoolMax} Sockets</span>
          </div>
          <div className="w-full h-1.5 rounded-full bg-slate-800 overflow-hidden">
            <div
          ...
          <div className="text-[11px] text-slate-400">Atlas Replica Set Primary Healthy</div>
        </Card>
      </div>
        ...
        <Card className="p-5 border-slate-800 bg-slate-900/80 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Active enterprise sessions connected across branch nodes.</p>
            </div>
          ...
          <div className="space-y-2.5">
            {metrics.activeSessions.map((sess) => (
              <div
                key={sess.id}
                className="p-3 rounded-2xl bg-slate-950/60 border border-slate-850 flex items-center justify-between gap-3 text-xs"
              >
                <div className="flex items-center gap-3">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 ring-4 ring-emerald-500/20 shrink-0" />
                  <div>
                    <p className="font-bold text-white">{sess.user}</p>
                    <p className="text-[11px] text-slate-400">{sess.branch} • IP: {sess.ip}</p>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-850 border border-slate-800 text-brand-300">
                    {sess.role}
                  </span>
                  <span className="text-[10px] text-slate-500 block mt-0.5">{sess.lastActive}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card className="p-5 border-slate-800 bg-slate-900/80 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Live latency benchmarks and request distribution.</p>
            </div>
          ...
          <div className="space-y-2.5">
            {metrics.apiEndpoints.map((api, idx) => (
              <div
                key={idx}
                className="p-3 rounded-2xl bg-slate-950/60 border border-slate-850 flex items-center justify-between gap-4 text-xs"
              >
                <div>
                  <span className="font-mono text-xs font-bold text-white">{api.endpoint}</span>
                  <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-400">
                    <span>Status: <span className="text-emerald-400 font-mono">{api.status}</span></span>
                    <span>•</span>
                    <span>Load: <span className="text-brand-300">{api.load}</span></span>
                  </div>
                </div>

                <div className="text-right font-mono">
                  <span className="text-xs font-bold text-emerald-400">{api.avgResponseMs} ms</span>
      ...
      <Card className="p-5 border-slate-800 bg-slate-900/80 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Infrastructure configuration and SLA availability compliance.</p>
          </div>
          <span className="text-xs text-slate-400">Last Optimization: <span className="text-white font-semibold">{lastOptimized}</span></span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-850">
            <span className="text-slate-500 block text-[10px]">CLUSTER REGION</span>
            <span className="font-bold text-slate-200">{metrics.clusterRegion}</span>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-850">
            <span className="text-slate-500 block text-[10px]">DATABASE VERSION</span>
            <span className="font-bold text-slate-200">{metrics.serverEnvironment}</span>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-850">
            <span className="text-slate-500 block text-[10px]">CACHE HIT RATIO</span>
            <span className="font-bold text-emerald-400">{metrics.cacheHitRatio}%</span>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-850">
            <span className="text-slate-500 block text-[10px]">CLUSTER UPTIME</span>
            <span className="font-bold text-brand-300">99.99% (98k+ secs)</span>
          </div>
        </div>
      </Card>
    </div>
