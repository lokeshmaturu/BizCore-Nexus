      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 bottom-0 w-full max-w-lg bg-slate-900/95 border-l border-brand-500/30 backdrop-blur-2xl z-50 flex flex-col shadow-2xl shadow-brand-500/10"
          >
            {/* Header */}
            <div className="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-brand-600 to-indigo-500 p-0.5 shadow-md shadow-brand-500/20">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-brand-500/20 text-brand-300 border border-brand-500/30 flex items-center gap-1">
                      <Sparkles className="w-2.5 h-2.5" /> Live OS
                    </span>
                  </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {QUICK_PROMPTS.map((qp, idx) => (
                        <button
                          ...
                          className="flex items-center justify-between p-3 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-700/60 hover:border-brand-500/40 text-left transition-all group"
                        >
                          <div className="flex items-center gap-3">
                            <qp.icon className="w-4 h-4 text-brand-400" />
                            <div>
                  ...
                  <div key={index} className="space-y-2">
                    {msg.role === 'user' ? (
                      <div className="flex items-start gap-2.5 justify-end">
                        <div className="max-w-[85%] rounded-2xl rounded-tr-none bg-brand-600 text-white p-3.5 text-xs sm:text-sm font-medium shadow-md shadow-brand-600/20 leading-relaxed">
                          {msg.prompt}
                        </div>
                      ...
                      <div className="flex items-start gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-brand-500/20 border border-brand-500/30 flex items-center justify-center shrink-0 text-brand-400">
                          <Bot className="w-4 h-4" />
                        </div>
                        <div className="max-w-[90%] rounded-2xl rounded-tl-none bg-slate-800/80 border border-slate-700/80 text-slate-200 p-4 text-xs sm:text-sm space-y-3 leading-relaxed shadow-lg">
                          <div className="font-semibold text-white flex items-center gap-1.5 border-b border-slate-700/60 pb-2">
                          ...
                          {msg.response?.keyMetrics && msg.response.keyMetrics.length > 0 && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
                              {msg.response.keyMetrics.map((km, i) => (
                                <div
                                  key={i}
                                  className="p-2 rounded-lg bg-slate-900/80 border border-slate-700/50"
                                >
                                  <div className="text-[10px] text-slate-400 truncate">{km.label}</div>
                                  <div className="text-xs font-bold text-white mt-0.5">{km.value}</div>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          {msg.response?.recommendations && msg.response.recommendations.length > 0 && (
                            <div className="space-y-1.5 pt-1 border-t border-slate-700/60">
                              ...
                              {msg.response.recommendations.map((rec, i) => (
                                <div
                                  key={i}
                                  className="text-xs p-2 rounded-lg bg-slate-950/60 border border-slate-800 text-slate-300 flex items-start gap-2"
                                >
              ...
              {isQuerying && (
                <div className="flex items-start gap-2.5">
                  <div className="w-7 h-7 rounded-lg bg-brand-500/20 border border-brand-500/30 flex items-center justify-center shrink-0 text-brand-400 animate-pulse">
                    <Bot className="w-4 h-4" />
                  </div>
                  <div className="p-3.5 rounded-2xl rounded-tl-none bg-slate-800/80 border border-slate-700/80 text-slate-400 text-xs flex items-center gap-2">
            ...
            {/* Input Form */}
            <form onSubmit={handleSubmit} className="p-4 border-t border-slate-800 bg-slate-950/80">
              <div className="relative flex items-center">
                <input
                  ...
                  disabled={isQuerying}
                  className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-4 py-3 pr-12 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors"
                />
                <button
                  type="submit"
                  disabled={!inputPrompt.trim() || isQuerying}
                  className="absolute right-2 p-2 rounded-lg bg-brand-600 text-white hover:bg-brand-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                >
